...
namespace SourceCode.Service
{
    public interface IProductService
    {
        ...
        IEnumerable<Product> GetProductFilterByCategories(int CategoryID, int min, int max, int discount);
        IEnumerable<Product> GetProductListByProductCategory(int ProductCategoryID);
        IEnumerable<Product> GetProductListByCategoryParent(int ProductCategoryParentID);
        IEnumerable<Product> GetProductListByUsageType(int UsageTypeID);
        ...
        IEnumerable<Product> GetProductListSold();
    ...
    public class ProductService : IProductService
    {
        ...
        public IEnumerable<Product> GetProductListByProductCategory(int ProductCategoryID)
        {
            IEnumerable<Product> listProduct = this.context.ProductRepository.GetAllData(x => x.ProCategoryID == ProductCategoryID && x.IsActive == true);
            return listProduct;
        }
        public IEnumerable<Product> GetProductListForHomePage(int productCategoryID)
        {
            IEnumerable<Product> listProduct = this.context.ProductRepository.GetAllData()
                .Where(x => x.ProCategoryID== productCategoryID && x.IsActive == true)
        ...
        public IEnumerable<Product> GetProductListByCategory (int CategoryID)
        {
            IEnumerable<Product> listProduct = this.context.ProductRepository.GetAllData(x => x.CategoryID == CategoryID && x.IsActive == true);
            return listProduct;
        }
        ...
        public IEnumerable<Product> GetProductListByUsageType(int UsageTypeID)
        {
            IEnumerable<Product> listProduct = this.context.ProductRepository.GetAllData(x => x.UsageTypeID == UsageTypeID && x.IsActive == true);
            return listProduct;
        }
        ...
        public IEnumerable<Product> GetProductFilterByCategories(int CategoryID, int min, int max, int discount)
        {
            IEnumerable<Product> listProduct;
            if (max != 0)
            {
                listProduct = this.context.ProductRepository.GetAllData(x => x.CategoryID == CategoryID && x.PromotionPrice >= min && x.PromotionPrice <= max && x.IsActive == true);
            }
            else
            {
                listProduct = this.context.ProductRepository.GetAllData(x => x.CategoryID == CategoryID && x.PromotionPrice >= max && x.IsActive == true);
            }
            return listProduct;
        }
        ...
        public IEnumerable<Product>GetProductListSold()
        {
            IEnumerable<OrderDetail> orderDetails = context.OrderDetailRepository.GetAllData();
            List<int> productIDs = orderDetails.Select(item => item.ProductID).ToList();
            if (productIDs.Count > 0)
            {
        ...
        {
            return context.ProducerRepository.GetAllData().Count();
        }
    }
}