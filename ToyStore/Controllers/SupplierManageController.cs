...
namespace SourceCode.Controllers
{
    ...
    public class SupplierManageController : Controller
    {
        ...
        public JsonResult CheckEmail(string email, int id = 0)
        {
            ...
            if (supplier != null)
            {
                if (supplier.ID == id)
                {
                    ...
                    {
                        status = true,
                        message = "Địa chỉ email hợp lệ và duy nhất"
                    }, JsonRequestBehavior.AllowGet);
                }
                else
                {
            ...
            {
                status = false,
                message = "Địa chỉ email hợp lệ và chưa tồn tại"
            }, JsonRequestBehavior.AllowGet);
        }
        ...
        private bool IsEmailValid(string email)
        {
            ...
            // Định dạng email với tên miền .vn hoặc .com
            return Regex.IsMatch(email, @"^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.(com|vn)$");
        }
