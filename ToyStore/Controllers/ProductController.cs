...
namespace SourceCode.Controllers
{
    public class ProductController : Controller
    {
        ...
        public ProductController(IProductService productService, IProducerService producerService, ISupplierService supplierService, IProductCategoryService productCategoryService, ICategoryService categoryService, IProductCategoryParentService productCategoryParentService, IUsageTypeService usageTypeService, IUserService userService, IQAService qAService, IProductViewedService productViewedService, IRatingService ratingService, IOrderDetailService orderDetailService)
        {
        ...
        public ActionResult Details(int ID)
        {
            ...
            var listProduct = _productService.GetProductListByCategory(product.ProCategoryID).Where(x => x.ID != ID);
            ViewBag.ListProduct = listProduct;
        ...
        [HttpGet]
        public ActionResult Categories (int ID, string keyword = "", int page = 1)
        {
            var listProduct = _productService.GetProductList().OrderByDescending(x => x.ViewCount).Take(5);
            ViewBag.ListProduct = listProduct;

            ViewBag.CategoryID = ID;
            Category categories = _categoryService.GetCategoryByID(ID);
            ViewBag.Name = "Phân loại: " + categories.Name;

            PagedList<Product> listProductPaging;
            if (keyword != "")
            {
                IEnumerable<Product> products = _productService.GetProductListByCategory(ID).Where(x => x.Name.Contains(keyword));
        ...
        public ActionResult ProductCategory(int ID, int page = 1)
        {
            ...
            ViewBag.Name = productCategory.Name;

            PagedList<Product> listProductPaging;
            IEnumerable<Product> products = _productService.GetProductListByProductCategory(ID);
        ...
        public ActionResult UsageType (int ID, int page = 1)
        {
            var listProduct = _productService.GetProductList().OrderByDescending(x => x.ViewCount).Take(5);
            ViewBag.ListProduct = listProduct;

            ViewBag.UsageTypeID = ID;
            UsageType usageType = _usageTypeService.GetUsageTypeByID(ID);
            ViewBag.Name = "Dùng cho " + usageType.Name;

            PagedList<Product> listProductPaging;
            IEnumerable<Product> products;
            if (ID != 3)
            {
                products = _productService.GetProductListByUsageType(ID);
            }
            else
            {
        ...
        public PartialViewResult FilterProductList(string type, int ID, int min = 0, int max = 0, int discount = 0, int page = 1)
        {
            ...
            if (type == "Categories")
            {
                ViewBag.Name = "Phân loại theo: " + _categoryService.GetCategoryByID(ID).Name;
                IEnumerable<Product> products = _productService.GetProductFilterByCategories(ID, min, max, discount);
